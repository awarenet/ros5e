...
export class Resting {
  ...
  static async _sybRest(
    ...
    type === restTypes.ext &&
      actor._getRestHitDiceRecovery(
        { maxHitDice: actor.system.details.level, type: typeKey },
        result,
      );
    ...
    actor._getRestResourceRecovery(
      {
        recoverShortRestResources: type === restTypes.short,
        recoverLongRestResources: type !== restTypes.short,
      },
      result,
    );
    await actor._getRestItemUsesRecovery(
      { type: typeKey, recoverDailyUses: newDay },
      result,
    );
  ...
  static async _displayRestResultMessage(result, actor) {
    ...
    switch (result.type) {
      case restTypes.short:
        restFlavor = "ROS5E.Rest.Flavor.Short";
        messageKey = "ROS5E.Rest.Results.Short";
        break;
      case restTypes.long:
        restFlavor = result.newDay
          ? "ROS5E.Rest.Flavor.LongOvernight"
          : "ROS5E.Rest.Flavor.Long";
        messageKey = "ROS5E.Rest.Results.Long";
        break;
      case restTypes.ext:
        restFlavor = result.newDay
          ? "ROS5E.Rest.Flavor.ExtendedRestOvernight"
          : "ROS5E.Rest.Flavor.ExtendedRestNormal";
        messageKey = "ROS5E.Rest.Results.ExtendedShort";
        break;
    }
    ...
    if (hitDice || hitPoints || corruption) {
      ...
      messageKey = `ROS5E.Rest.Results.${length}Full`;
    }
  ...
  static _restHpGain(actor, type) {
    ...
    switch (type) {
      ...
      case restTypes.long:
         ...
         */
        const hitDieSize = Object.entries(actor.classes).reduce(
          (acc, [key, cls]) => {
  ...
  static _getRestHitPointUpdate(actor, type, result) {
    ...
    const clampedFinalHp = Math.min(
      rawHpGain + actor.system.attributes.hp.value,
  ...
  static _corruptionHealUpdate(currentCorruption, amount, result) {
    ...
    result.actorUpdates[game.ros5e.CONFIG.PATHS.corruption.temp] = newTemp;
  }
  ...
  static async expendHitDie(actor, denomination) {
    ...
    // Otherwise, locate a class (if any) which has an available hit die of the requested denomination
      ...
      cls = actor.system.attributes.hd.classes.find((i) => {
    ...
    if (!cls) {
      ui.notifications.error(
        game.i18n.format("DND5E.HitDiceWarn", {
          name: actor.name,

...
export class SybRestDialog extends HandlebarsApplicationMixin(ApplicationV2) {
  ...
  static PARTS = {
    form: {
      template: `modules/ros5e/templates/apps/rest.hbs`,
    },
  };
  ...
  async _prepareContext(options) {
    ...
    context.newDay = context.promptNewDay;
    ...
    /* 
		...
		*/
    context.denominations = Object.entries(context.availableHD.bySize)
      .map(([size, value]) => {
        return {
          hitDice: size,
          label: size,
  ...
  async _onReduceCorruption(event) {
    ...
    if (!hdSelect) return;
    const denom = hdSelect.value;
    (await Resting.expendHitDie(this.actor, denom)) &&
      (await Resting.corruptionHeal(
        this.actor,
